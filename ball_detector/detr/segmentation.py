...
class PostProcessSegm(nn.Module):
    ...
    def forward(self, results, outputs, orig_target_sizes, max_target_sizes):
        ...
        max_size = max_target_sizes.max(0).values
        max_h, max_w = int(max_size[0]), int(max_size[1])

        masks = outputs["pred_masks"].squeeze(2)
        masks = F.interpolate(
            masks, (max_h, max_w), mode="bilinear", align_corners=False
        )
        masks = masks.sigmoid_().gt_(thresh)

        for idx, res in enumerate(results):
            img_h, img_w = max_target_sizes[idx]
            tgt_h, tgt_w = orig_target_sizes[idx]

            m = masks[idx, :, :img_h, :img_w].unsqueeze(1).float()
            m = F.interpolate(m, (tgt_h, tgt_w), mode="nearest").to(torch.uint8)
